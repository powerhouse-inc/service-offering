...
| **Validation** | Slack: prorated credit, non-refundable, auto-applied. Zoom: no credit. Stripe: credit on next invoice. We chose Slack model. **Google Workspace** (Mar 2026 invoice): seat reduction from 3 → 1 mid-cycle. Prepaid €48.60, actual usage €39.19, difference (€9.41) carried as credit balance. Same proration-as-credit pattern as D-2, applied to quantity delta rather than whole group removal. |
...
| **Validation** | Stripe: invoice timing doesn't shift subscription period. Late settlement: overage window up to `nextBillingDate`. Early settlement: overage window up to `settlementDate`. |
...
| **Validation** | Slack: credits non-refundable, auto-applied to future charges. Stripe: credit balance carries forward. |
...
| **Validation** | AWS: volume tiers reset monthly, storage persists. Slack: seat count persists. |
...
| D-4: Cycle boundaries stay fixed | 00:40:05: "close the billing cycle prematurely... or lagging 6 weeks" | **Stripe**: prorations happen on plan changes, not on invoice timing. Invoice date doesn't shift subscription period. | `currentBillingCycleStart: DateTime` on state. `settlementDate` on input. | Late: overage window to `nextBillingDate`. Early: to `settlementDate`. No date drift. |
...
| D-7: Negative balance carries forward | 01:04:27: "if payment comes in you update credit" | **Slack**: credits non-refundable, auto-applied. **Stripe**: credit balance carries forward. | `calculateAmountOwed()` returns raw difference, no floor. | Refunds are a Stripe concern. Capping hides surplus. |
